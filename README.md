...
6. **Briefs other agents** by adding a `<!-- clud-bug-start --> ... <!-- clud-bug-end -->` block to `AGENTS.md` — and only to `AGENTS.md` (creating it if missing; it's the cross-tool canonical). Per-tool files that already exist — `CLAUDE.md`, `GEMINI.md`, `.github/copilot-instructions.md`, `.cursorrules`, `.windsurfrules`, `.clinerules`, `.continuerules`, `.cursor/rules/*.md` — get a one-line redirect stub pointing back at `AGENTS.md`, never a copy of the instructions. One source, seven pointers, one drift rate. If a file already redirects (a `@AGENTS.md` import, logmind's stub, or any link to `AGENTS.md`) it's left untouched, and anything you hand-wrote around our block is preserved. Re-runs replace the block in `AGENTS.md` in place. Files you didn't already have are left uncreated — no proliferating stubs.
...
- **`gh pr comment` is disabled by default.** Without `--allowedTools` whitelisting it, Claude runs, thinks, and exits silently. clud-bug's generated workflow includes the right allowlist — and every post site routes through one `upsert_review_comment` helper (identity + `<!-- written-by: ... -->` marker, `--paginate`d comment search), so a PR gets **one** review comment, edited in place on every push — including a busy PR carrying 100+ other comments — never a fresh one stacked on top (SPEC 2.0 §4.3).
- **Skills are not auto-loaded from anywhere.** If you don't ship `.claude/skills/*` in your repo, Claude reviews with zero project context. clud-bug installs a curated set so the review is actually project-aware — and the allowlist documents the intended capped read recipe, `Bash(head -c 8192 .claude/skills/*/SKILL.md)` (the cap named in the grant itself, not a `Bash(head:*)` wildcard the model could hand any byte count to) and drops the grants that were pure dead weight next to it — the uncapped `Bash(cat .claude/skills/*/SKILL.md)` and `Bash(git show:*)` (clud-bug#332). None of that binds `MAX_SKILL_BYTES`, though: `head` and `cat` are Claude Code built-in read-only commands that run with no permission check at all, and the allowlist's own `Bash(git diff:*)` / `Bash(gh api repos/:*)` grants (kept for diff-fetching) can read the same skill file's full bytes by a different route — `--allowedTools` cannot cap or block any of it, on this or any other pattern (see [Claude Code docs, "Read-only commands"](https://code.claude.com/docs/en/permissions)). What actually binds a byte cap is truncating what the model is *given* before it gets a shell at all — the hosted App's `buildReviewPrompt` does this for its own path; this workflow path doesn't, and #332 stays open for that.
- **The `clud-bug` CLI the workflow invokes is resolved from OUTSIDE your checkout.** Every generated workflow (including the fork-notice job, which has no checkout at all) installs the pinned, published package into `$RUNNER_TEMP` before checking out the PR, then calls it by absolute path — so a PR cannot ship its own `node_modules/clud-bug` and have the review run that code instead of the real CLI (SPEC 2.0 §6.3).
- **The strict-mode gate's own comment search paginates.** A single windowed fetch could lose the bot's own review comment on a PR busy enough to carry 100+ other comments, reporting "no review found" and passing a PR with real critical findings. The gate walks every page and selects by identity alone — never the `<!-- written-by: ... -->` marker the comment helper stamps, since the gate's own documented default `bot-login` (`claude[bot]`, third-party `anthropics/claude-code-action`) never stamps one, and requiring it would fail the gate open under that default — never by how recently a comment happened to be created relative to everything else on the PR.

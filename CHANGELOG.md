...
- **`check-doc-links.yml` and `regen-timeline.yml` called `thrillmade/setup-logmind@v1.0.1` with no `token:`, so its release-lookup API call ran unauthenticated and hit GitHub's anonymous rate limit — a 403 that failed both checks on PR #354.** Both call sites now pass `token: ${{ github.token }}`, matching the form `logmind-self-update.yml` already used (#329); a new `test/setup-logmind-token.test.js` asserts every `thrillmade/setup-logmind` step under `.github/workflows/` carries one.

...
export function rollup(reviews) {
  ...
  return { total, perRepo, perModel, trend30d, outliers };
}
...
function computeTrend(reviews) {
  if (reviews.length === 0) {
    return { current: 0, previous: 0, slopePct: 0 };
  }
  const now = Date.now();
  ...
  const slopePct = previous > 0 ? ((current - previous) / previous) * 100 : 0;
...
export function formatRollup(rollup, opts = {}) {
  ...
  const trendStr = trend.slopePct === 0
    ? '(no prior window)'
    : `${trendArrow} ${trend.slopePct.toFixed(0)}% MoM`;

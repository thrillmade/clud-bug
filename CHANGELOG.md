...
## [Unreleased]
- **A review pass now resolves to a roster entry, not a reviewer tier (#268, SPEC §2.4).** New `readRoster(repoRoot)` (`src/core/roster.ts`) parses every `.claude/agents/*.md` frontmatter into `{ name, description, model?, tools?, color?, trigger?, file }`; a malformed entry (missing `name`/`description`, unparseable YAML, a `tier` field, or a `name` that doesn't match its filename) is reported in a `problems` list naming the file and the reason — never silently dropped, and no entry's body is ever read past its frontmatter or sent anywhere. `resolveReviewPasses` (`src/core/review-plan.ts`) matches a resolved pass's role to a roster entry by exact `name`; a match's `model` is what actually dispatches the pass when the entry pins one — `model` is optional, so a model-less match still surfaces (`rosterFile` is set either way) but leaves the pass on its fallback model. The `reviewPasses.roles` config field and its beetle/wasp/mantis tiers are the pre-roster mechanism this replaces — **deprecated**, kept only as the fallback for a role the roster doesn't cover, slated for removal once the catalog ships rosters (agent-skills#180). `clud-bug review-prompt` prints which roster file each pass resolved to ("roster: `.claude/agents/<name>.md`") and prints a malformed roster file as a warning on stderr, never a crash. The baseline pass is unaffected — nothing here touches pass `count`, only which model runs it.
- **`clud-bug config list|get|set|unset` — one schema owns every setting (#271, SPEC §1.6).** `CONFIG_KEYS` in `src/core/config-schema.ts` maps each SPEC name to its on-disk key, domain, default, owner and citation, and the command is the only writer: `review.strict_mode` and `review.auto_fix` are humans-only and are refused with a distinct exit code; unknown keys get did-you-mean; a manifest that cannot be parsed is left byte-identical (the previous reader returned an empty manifest on a parse error, so the next write deleted the user config). The refusal stops an agent that asks; the base-ref re-read and the visible diff hunk remain the real guard.
...
- **Workflow templates stated a `MAX_SKILL_BYTES` of `4000` — half the library's own `DEFAULT_MAX_SKILL_BYTES` and below SPEC 2.0 §2.1's 8 KiB recommendation (#301, #305).** Every skill written to the `thrillmade/agent-skills` house structure landed over that budget and was silently truncated. The three workflow templates now render `MAX_SKILL_BYTES` from `DEFAULT_MAX_SKILL_BYTES` (`src/core/prompt-builder.ts`) at `clud-bug init`/`update` time instead of stating an independent number — one owner for the fact, not two. The review prompt's own prose (`src/core/prompts.ts`) told the model the same cap defaulted to "4,000 bytes" in a second, hand-copied number that the first fix moved past without updating; it now interpolates `DEFAULT_MAX_SKILL_BYTES` directly, so the number the model reads and the number the shell enforces can never drift apart again. #305 also found the App-side prompt builder never reads a skill's `references/` subdirectory (confirmed: it destructures only `{slug, frontmatter, body}` from each loaded skill); that was already true and is now documented in README's "Adding your own skills" section rather than left for an author to discover via silent truncation.

- **A PR-controlled workspace could still hand its own reviewer trusted instructions through any file the base ref's launch-root memory files `@`-import — and the first fix for that was itself exploitable (#291).** #260/#288 pinned `.claude/skills` to the PR's base ref before the reviewer runs; #291 asked whether `CLAUDE.md`, the rest of `.claude/`, and `AGENTS.md` needed the same treatment. `anthropics/claude-code-action`'s own `restoreConfigFromBase` already restores `CLAUDE.md`, `CLAUDE.local.md`, and the whole `.claude/` tree from the base branch before its CLI reads anything (present since v1.0.133, the version this repo already pins) — but that only fixes those files' own bytes. Claude Code then resolves their `@`-import lines against whatever sits in the workspace, which is still the PR's own content: `AGENTS.md` (this repo's own `CLAUDE.md` is exactly `@AGENTS.md`), and recursively, anything those imported files themselves import. The "Pin review skills to the base ref" step now walks that graph with one loop, from the launch roots Claude Code documents — `CLAUDE.md`, `.claude/CLAUDE.md`, `CLAUDE.local.md`, plus `AGENTS.md` — to the same four-hop maximum that resolver has, and restores each path from the base ref or leaves it absent.

  Three things the loop gets right that an earlier pass did not, each reproduced end-to-end against the rendered shell before it was changed: **discovery reads the base ref and nothing else.** Every path found becomes an argument to a delete, so reading "the workspace copy or the base-ref copy" made every candidate past the seeds one the pull request chose — a PR `CLAUDE.md` containing `@.git` deleted the git directory, which makes the strict-mode gate's `git show origin/<base>:.claude/skills/.clud-bug.json` fail, and that gate exits 0 when it cannot read its own config: a pull request disabling strict mode on itself, through the step meant to stop it. **The filesystem is touched through one primitive** that denies `.git` and everything under it outright, unlinks a symlinked leading segment — the link itself, never what it points at — refuses a non-directory leading segment, refuses a leaf that is a directory, deletes with `rm -f` rather than `rm -rf`, and restores only a path that is a **blob** in the base ref. A lexical path test cannot see a `..` that lives inside a committed symlink: with `docs` linked out of the checkout, `docs/victim.md` carries none and the delete landed outside the repository; with `docs` linked to a directory the PR itself committed, *refusing* the pin was worse than useless — that link resolves inside the working directory, so Claude Code asks no approval question and loads the PR's bytes as project instructions. Unlinking the link closes both shapes and lets the base blob be restored into a real directory. `.git` needed a rule of its own because bare `.git` survived only by accident (it happens to be a directory) while `.git/HEAD` is a file: one benign sentence in the *base* ref — `Never write @.git/HEAD by hand` — deleted git's own metadata, and a `git show origin/<base>:…` that cannot run is exactly how the strict-mode gate turns itself off. The fail-closed `rm -rf .claude/skills` carries the same link-only guard, for the same reason: `rm -rf` resolves an intermediate symlink, so a PR that committed `.claude` as a link had the runner recursively delete a tree outside the checkout. **The parser follows the imports Claude Code actually follows** — `@path` anywhere on a line, not only alone on one, and never inside a Markdown code span or fenced block — resolved relative to the file that declared the import rather than to the working directory. A whole-line pattern dropped every inline form the memory docs show (`See @README for project overview`, `- git workflow @docs/git-instructions.md`), so the PR still supplied those instruction files.

  A later pass found the path handling itself was three separate holes, each reproduced against the rendered shell before it was changed. **A `..` is now normalised, not refused.** The memory docs define the import Claude Code puts behind its approval dialog as one that is "external when its path resolves outside your working directory" — so `@../docs/policy.md` in `.claude/CLAUDE.md` resolves to `docs/policy.md`, inside the checkout, and the CLI loads it with no question asked. Refusing to pin it was fail-open: the step announced a rejection and left the pull request's own `docs/policy.md` at the path the reviewer reads as instructions. The resolver now normalises the joined path (`.` dropped, `..` popping the segment before it) and refuses only a normal form that climbs out of the checkout. **The same normalisation makes the delete and the restore address the same object.** `rm -f docs/./b.md` succeeds where `git cat-file -t "<sha>:docs/./b.md"` cannot resolve, so an import written `@./b.md` deleted the base ref's own file and then reported that the base ref "declares no `docs/./b.md`" — base content gone, the reviewer told a path was absent that was never asked about. **And one unpinnable path no longer ends the loop.** The step runs under `bash -e`, where any single refusal that is not a guarded `return` aborts the whole step and every later import keeps the PR's bytes. A path the base ref carries as a **directory** is now refused before anything is deleted (an instruction file is never one, and "declares no `adocs`" was a lie about a directory that is right there in the base ref); the `rm` itself is guarded too, which is defence in depth rather than a PR-reachable shape — the base-type check and the probe loop intercept every directory shape a pull request can commit, so what can still fail there is the workspace's own (an unwritable parent, a read-only mount), and `test/base-ref-instruction-surface.test.js` case 15 pins it with exactly that. The step also declares `shell: bash` now instead of inheriting the interpreter and the `-e` its body depends on.

- **The strict-mode gate turned a failed read of its own config into "strict mode disabled for this run" and a green check (#291).** `.github/actions/strict-mode-gate/action.yml` read the base manifest with `git show "origin/<base>:.claude/skills/.clud-bug.json"` and treated *any* failure as an opt-out — a shallow clone, a missing `fetch-depth: 0`, or a deleted `.git` was enough to switch strict mode off for a repository whose base ref says `strictMode: true`. This is the fail-open the `#291` pin step's own security comment cites as its reason to deny `.git` outright, and it was still live one layer down. The gate now resolves `origin/<base>` first and **fails the check** when it will not resolve; only a base ref that resolves and genuinely carries no manifest is an opt-out. Resolving the ref is only half of that, and the next two fixes for it got the other half wrong in the same shape, twice. The first asked `git cat-file -e` whether the manifest OBJECT was in the store, which gives a declared-but-unreadable manifest — the ref and every tree intact, the blob gone — the same answer as one that was never committed, so the fail-open survived one layer in. The second asked `git rev-parse <commit>:<path>` instead, on the theory that walking the trees needs no blob — true for a corrupt leaf blob, but rev-parse fails exactly the way `cat-file -e` did when the corrupt object is an intermediate TREE (`.claude/skills` itself) rather than the manifest blob, so the fail-open survived a second layer in. The question is whether the base ref *declares* the path, which `git ls-tree <commit> -- <path>` answers without needing any object on the path to be readable: empty output and exit 0 mean the path genuinely isn't there, a nonzero exit means some tree along the way couldn't be read, and only the first of those is the opt-out. The read that follows is then allowed to fail, and does. The per-skill check-run step reads the same manifest the same way — a `strictSkills` check-run that silently never appears is a required check that silently never blocks.

- **actionlint never actually linted the strict-mode gate it ships (#291).** Passed the file directly it fails on schema before reading any content (`"jobs" section is missing` — an `action.yml` is not a workflow); the shipped templates' `uses: thrillmade/clud-bug/.../strict-mode-gate@vX` is a *remote* reference, which actionlint resolves from its own bundled popular-actions data rather than reading this repo's file at all. Neither path had ever reached `.github/actions/strict-mode-gate/action.yml`'s own `inputs:`/`description:`/`runs.using` metadata — confirmed by deleting the required `github-token` input declaration and watching the existing two-file `Lint own workflows` loop pass clean. That step now generates a throwaway workflow with a *local* `uses: ./.github/actions/strict-mode-gate` step — the one shape actionlint does read a local action's metadata through — and lints it alongside the other two. It does not check the composite's `run:` steps (confirmed unsupported: a deliberately broken `run:` body produced zero findings through this same local-uses route); the two guards above are covered by `test/strict-mode-gate-base-ref.test.js` instead.

- **ci.yml's own actionlint job only rendered and linted `templates/workflow*.yml.tmpl`, so `audit.yml.tmpl`, `fork-notice.yml.tmpl`, and `self-update.yml.tmpl` shipped unlinted — excluded by name, not by anything those three actually need done differently.** Widened to every `templates/*.yml.tmpl`; none of the three references `{{REVIEW_PROMPT}}`, so the existing render call (which always supplies it) is a no-op for them. Rendering and linting all six for real surfaced two real findings in `self-update.yml.tmpl` that had never been checked before — three consecutive `echo ... >> "$GITHUB_OUTPUT"` redirects (SC2129; grouped into one `{ ...; } >> "$GITHUB_OUTPUT"`, the same idiom `workflow.yml.tmpl` already uses) and a Markdown-backtick false positive in a `printf` body (SC2016; same shape as the pre-existing disable below) — and one in `fork-notice.yml.tmpl`'s "Announce the skip" step that had escaped every prior pass for the same reason: the same Markdown-backtick SC2016 false positive, now silenced with the same `# shellcheck disable=SC2016` comment `workflow.yml.tmpl` already carries for its own copy of this string. All six templates now render and lint clean through this loop's exact commands.

- **Three source comments cited `SPEC §1.10`, a section SPEC 2.0 does not have.** Its headings run 1.8 → 2.1, so the skill frontmatter contract a reader was being sent to is §2.1. `src/core/skills.ts` now cites `SPEC 2.0 §2.1` at all three sites, and a test walks `src/**/*.ts` refusing any bare `§1.10`. The version-stamped `§1.10.1 v0.5.1+` citations elsewhere in that file are a different thing — they record what an older SPEC said, which stays true, and they stay. What makes those legitimate is the stamp, not the extra digit: `skills.ts:302` carried a `§1.10.1` without one, reading as a live section of the SPEC 2.0 the rest of the file cites, so it is stamped now and a second test refuses an unstamped `§1.10.1` anywhere under `src/`.

- **The `review` job resolved the `clud-bug` CLI via bare `npx`, letting a committed `node_modules/clud-bug` run its own code as the gate (#331; SPEC 2.0 §6.3).** `npx --yes clud-bug@<version> <verb>` scans `node_modules` upward from the current working directory before ever asking the registry; the job's `actions/checkout@v6` carries no `ref:`, so on `pull_request` that directory is the merge ref — a workspace holding the PR's own content. A PR that commits `node_modules/clud-bug` (permitted even where `.gitignore` lists it, via `git add -f`) made every one of the CLI's call sites run the PR's own code, with `GH_TOKEN` and `ANTHROPIC_API_KEY` in hand, to post the very `clud-bug-review` check-run that gates the same PR — confirmed directly: a fixture `node_modules/clud-bug` whose bin prints a marker is exactly what `npx --yes clud-bug@<pinned>` returns from such a workspace, offline. Fixed at the resolution, once: a new "Install clud-bug CLI (pinned, isolated)" step, running BEFORE `actions/checkout` in every job that calls the CLI, installs the pinned published package into `$RUNNER_TEMP` — outside the checkout, and before it exists, so a workspace-level `.npmrc` a PR might commit can't steer the install either — and exports its absolute path as `CLUD_BUG_BIN` via `$GITHUB_ENV`. Every `clud-bug <verb>` call site in all three workflow templates now invokes `"$CLUD_BUG_BIN"`; confirmed absolute-path resolution is unaffected by the same poisoned workspace that fooled `npx`. `templates/fork-notice.yml.tmpl` (the `pull_request_target` fork-notice job, no checkout at all) still had one bare `npx` call site — same fix, same isolated install step. And the regression pin itself was line-oriented (`[^\n]*`, stopping at the first `\n`), so an `npx` call wrapped across several lines with shell `\` continuations — `npx` on one line, `clud-bug@<version>` on a later one — would have slipped past it; the pattern now walks zero or more `\`-continued lines before the one the version pin lands on.

- **`--allowedTools` granted an uncapped `cat` of a skill body alongside the capped `head`, so `MAX_SKILL_BYTES` was a suggestion (#332; SPEC 2.0 §2.1: "A skill body SHOULD stay under 8 KiB").** The system prompt tells the model to read a skill capped, via `head -c "$MAX_SKILL_BYTES" .claude/skills/<name>/SKILL.md` — but `Bash(cat .claude/skills/*/SKILL.md)` sat in the same `--allowedTools` list, matching the identical glob, uncapped. A first pass dropped that `cat` grant and left `Bash(head:*)` in its place, reasoning it was "the recipe the system prompt teaches" — that doesn't hold: `Bash(head:*)` is a wildcard on the whole command line, so `head -c 999999999 .claude/skills/x/SKILL.md` was exactly as permitted as the documented `head -c "$MAX_SKILL_BYTES"` one. The cap was still only ever prompt text. Fixed for real: the one grant that can read a skill body names the cap in the command itself, `Bash(head -c 8192 .claude/skills/*/SKILL.md)` (8192 = `DEFAULT_MAX_SKILL_BYTES`, pinned by a test rather than templated — `scripts/run-benchmark.mjs` extracts `--allowedTools` from the raw, un-rendered template, so a `{{MAX_SKILL_BYTES}}` token here would ship inert). `Bash(head:*)` is gone; so is `Bash(git show:*)` — added "defensively" for base-ref reads that, in fact, all happen in workflow-level `run:` steps outside this allowlist entirely, so left in place it was just another uncapped path to a skill body's full git-object content. The diff/comment budget recipes (`head -c "$MAX_DIFF_BYTES"` / `"$MAX_COMMENT_BYTES"`, and their tee-hint doublings) keep working via 4 literal, path-less grants — none of them can resolve any file, let alone one under `.claude/skills`. `Bash(cat .claude/skills/.clud-bug.json)` stays untouched, since that's a small, fixed manifest, not the unbounded skill body §2.1 is about.

  Correction to "fixed for real" above: it wasn't. `head` and `cat` are Claude Code built-in read-only commands, run with **no permission check at all**, so `--allowedTools` cannot cap or block either one on any pattern — and even setting built-ins aside, this same allowlist keeps `Bash(git diff:*)` and `Bash(gh api repos/:*)` for legitimate diff-fetching, and both reach a skill body's full bytes uncapped by a different route (e.g. the GitHub Contents API, or a diff against `/dev/null`). Naming the byte count in the grant narrows what the allowlist *documents* — the intended read recipe — not what the model can run; it does not enforce the cap. What binds `MAX_SKILL_BYTES` is the prompt builder's own truncation of what the model is *given*, not an allow-list over what it's permitted to run: the App's `buildReviewPrompt` (`src/core/prompt-builder.ts`) does that for its own path by truncating the skill body before it ever reaches the prompt. This workflow path hands the model a shell and a recipe instead, and no shell permission list binds a model that already has one. #332 stays open for real enforcement on this path.

- **The strict-mode gate's own comment fetch was a single windowed page, so its review comment could age out of view on a busy PR and the gate failed OPEN (HIGH).** `.github/actions/strict-mode-gate/action.yml` fetched `?sort=created&direction=desc&per_page=100` — the newest 100 comments, one page, no `--paginate`. `upsert_review_comment` (below) edits ONE comment in place per PR, so its `created_at` never advances while unrelated discussion piles up around it; once a PR carries more than ~100 comments after the bot's own, a single windowed page stops returning it at all. The gate then saw no review comment ("NONE" — advisory) and passed a PR that actually had critical findings. Fixed: `gh api --paginate` walks every page (correctly, via `--jq '.[]' | jq -s` — `gh api --paginate` alone prints each page as its own top-level JSON value, not one merged array, so piping it straight into an array-literal `jq` program breaks on the second page). A first pass narrowed the merged result to comments carrying the identity **and** the `<!-- written-by: ... -->` marker `upsert_review_comment` stamps — wrong, and a second fail-open in the same step: that marker is `upsert_review_comment`'s own anchor for the identity IT posts under, and this gate's documented *default* `bot-login`, `claude[bot]` (third-party `anthropics/claude-code-action`), never stamps it, so the marker-required filter found zero comments under the action's own default usage and the gate stayed advisory regardless of findings. The `jq` filter narrows on identity alone (`.user.login == $bot`) and leaves header selection to `classifier.mjs`'s existing `selectReviewHeader`, which already re-derives identity per comment. Proven end-to-end against the real `classifier.mjs`, a real git repo, and a 151-comment fixture with the review comment OLDEST (i.e. the one a recency-windowed fetch drops first) and no written-by marker in its body (matching `claude[bot]`'s real shape): the fixed script correctly fails the check with `bot-login` both omitted (default) and set explicitly; the pre-fix single-page shape, and a marker-required re-mutation of the fixed shape, both reproducibly pass it.

- **The Action posted a new review comment on every push instead of editing the existing one, so a PR with N pushes accumulated N review-status comments (#259 item 1; SPEC 2.0 §4.3: "One comment per pull request, rewritten in place... MUST NOT post a second one"; §2.6 names the runtime contract, `upsert_review_comment`).** Four `gh pr comment ... || true` call sites in the review job — the same-repo bot-skip advisory, the structured-output render, and both Layer 6 fallback branches — each posted independently, with the skip advisory's own ad hoc dedup query the only guard against duplication, and only for that one message shape. A new "Write review-comment helper" step defines ONE bash function, `upsert_review_comment <pr> <repo> <body>`, that every one of the four sites now sources and calls: it finds the bot's own prior comment by the `<!-- written-by: ... -->` marker `clud-bug/core`'s shared renderer (`review-writeback.ts`) already defines for this role — stamped with `github-actions[bot]`, the identity this job actually posts under, not the App's `clud-bug[bot]` — and edits it (`gh api --method PATCH`) in place, creating one only when none exists yet. Proven end-to-end: two calls against the same fake PR produce exactly one comment, the second call editing rather than duplicating it. The helper's own comment search had the SAME residual as the gate above — `?per_page=100` with no `--paginate`, so a PR that already carried 150 other comments before the bot's own first posted could never find it again, and every later run created a fresh comment instead of editing it. Fixed the same way: `--paginate`. Proven with a 151-comment fixture (150 noise + the bot's own comment last): a second run edits it in place; the pre-fix shape against the same fixture creates a duplicate.

  A second residual in the SAME lookup line, in all four templates (`workflow.yml.tmpl`, `workflow-ts.yml.tmpl`, `workflow-py.yml.tmpl`, `fork-notice.yml.tmpl`): `existing=$(gh api --paginate ... | tail -n1) || existing=""` lands a genuine lookup FAILURE (a 502, a secondary rate limit, a page dropped mid-walk) in the exact same `existing=""` a real "not found" produces — the `||` only fires on `tail`'s own exit, which succeeds on empty input regardless of whether `gh api` upstream of it ever ran at all — so a failed lookup fell through to CREATE, the second comment §4.3's MUST forbids. The exit status is now captured on its own command substitution (`|| lookup_status=$?`), never through a pipe that can mask it; a non-zero status emits `::warning::` and returns without creating, before the found/not-found branch is even reached. Proven with a fake `gh` whose lookup call always fails after streaming a first page (matching a real `gh api --paginate --jq` mid-walk HTTP error): the fixed helper does not create a comment and fails the step; the pre-fix shape, against the same fixture, creates one.

- **The 'Fallback summary (structured_output empty)' step could overwrite Guard's "nothing here was checked" advisory with a synthetic reviewed-and-clean comment.** On a same-repo bot PR with no `ANTHROPIC_API_KEY`, Guard posts its own skip advisory and exits 0 (success), then skips the action step (`if: steps.guard.outputs.skip != 'true'`) — leaving `structured_output` empty. The Fallback summary step's `if:` only checked `success() && structured_output == ''`, both still true, so it fired too and — via the SAME `upsert_review_comment` marker — replaced the accurate "nothing here was checked" advisory with a "## 🐛 Clud Bug review / 0 critical · 0 minor" comment that reads as reviewed-and-clean. Fixed: gated on `steps.guard.outputs.skip != 'true'` too, the exact clause the action step above it already uses.

### Changed

- **Documented three items #291's fix does not close, named rather than left for the comment to overclaim.** (a) On a `pull_request` trigger the forge reads the workflow file itself from the PR's merge commit, so a PR that edits it has already replaced the job — including the base-ref pins above — before either executes (SPEC 2.0 §6.3). (b) Nested `CLAUDE.md` / `CLAUDE.local.md` files anywhere else in the repository tree, which Claude Code loads on demand whenever the reviewing session reads a file in that subdirectory, and the `@`-imports of `.claude/rules/**.md` — covered by neither claude-code-action's `SENSITIVE_PATHS` (root-level names only; `.claude` is its one recursive entry, so it restores the rules files but not what they point at) nor this loop. (c) An import the step refuses — absolute, `~`-relative, or one whose normal form climbs out of the checkout — is announced and then left alone, so the PR's copy of that target keeps PR bytes: fail-visible, not fail-closed. The approval dialog no non-interactive CI session answers covers the third of those ("external when its path resolves outside your working directory"); an absolute path pointing back *into* the checkout is refused here anyway, because the runner's workspace prefix is not something the step can match on reliably. It declines rather than guesses, and proves nothing either way.

  The loop's depth is not a residual: it runs the four hops the memory docs give the resolver it shadows ("Imported files can recursively import other files, with a maximum depth of four hops"), a chain four links long is pinned end to end by `test/base-ref-instruction-surface.test.js` case 6, and a test fails if the bound in the shell and the number the comment quotes ever stop being the same one. Shallower would leave a chain the CLI still walks; deeper would pin a file the CLI never loads to base bytes the PR legitimately changed.

### Added

- **The planted-defect benchmark is wired to something (#270).** SPEC 2.0 §8.2 requires it — "A reviewer MUST periodically be given a change carrying a defect whose presence is known in advance, and MUST report it" — and the 20-scenario corpus under `benchmark/` was scored once by hand and wired to nothing: the `r7-score-benchmark` workflow its README cited did not exist. Measured before this change: `ls .github/workflows/ | grep -E 'score|benchmark'` → 0 hits, control probe `ci` → 1 file, so the search itself worked. The README sentence claiming that workflow is deleted rather than struck.

  - `benchmark/scenarios/<id>/answer.json` — the answer key in machine-readable form for all 20 scenarios: the planted site (`file` + `lineRange`, plus `acceptAlso` for the second equally-correct site of a cross-cutting defect) and whether a correct review reports a `finding` or stays `clean`. Withheld from the reviewer alongside `SCENARIO.md` and `reproduce.mjs`.
  - `src/core/benchmark-score.ts` — pure scorer. A catch must be a 🔴 **located at the planted site** (the benchmark measures what the merge gate would do, and only a 🔴 blocks); on a decoy a 🔴 anywhere is a false positive; several reviewers on one scenario resolve conservatively — all must catch it, one flagging a decoy is a false positive.
  - `scripts/run-benchmark.mjs` — materialises each scenario as the two-commit repo the Action would review (pre-existing modules in the base commit, `module.mjs` as the change, skills pinned from the base ref per #288), then runs a headless reviewer over it. The tool allow-list, the default model, the thinking budget and the review prompt all come from **one** template — `pickTemplate(['javascript'])`, the JS/TS workflow, because every scenario is a `module.mjs` and that is the recipe `clud-bug init` would install here — rather than restated, because a benchmark that drifts from the shipped recipe measures something nobody runs. The template it read is recorded in `config.workflowTemplate`. Where it cannot mirror the Action (one headless pass, no PR context or `gh`, no inline-comment tool, a local two-commit repo) is recorded in the result's `invocation` and therefore printed in every published copy of the score. `--dry-run` exercises the plumbing with no key and no network, and may not write the published results dir — the destination is resolved and compared to it, not tested for having been named, because `--out benchmark/results` is an explicit `--out` and overwrote the published score with a fake one. Neither may a `--suite` subset, whose filtered-out scenarios leave the counts rather than becoming `unverified`.
  - **The operator cost cap (SPEC 2.0 §4.9) is checked *before* each call**, charged for every call that started — a reviewer that errored or timed out was billing until it died, so only one that never started is free — and passed to the reviewer as `--max-budget-usd` so a single call cannot outspend what is left. Every scenario left unrun is recorded `unverified` — never clean. The published wording names the remainder when there is one (§2.8: an elision MUST leave a marker saying how much was cut — the move §8.2 makes for a partial re-review).
  - **The corpus is run in a seeded, class-interleaved order.** Directory order put all six decoys in one block at the end, and the scheduled trigger passes no seed, so any cost stop inside the planted defects scored zero decoys — and precision over zero decoys is 100% by arithmetic. The order is now shuffled from a seed that defaults to the run id and is always recorded in `config.seed`, then the planted defects and the decoys are interleaved in proportion, so every prefix samples both halves. The result file still reads in corpus order whatever order it was produced in.
  - **A reviewer payload is validated before it is scored.** The `structured_output` branch is schema-checked by the CLI; the fenced-`result` branch was whatever the model typed, so a payload with no `critical_findings` scored as an empty finding set — indistinguishable from a reviewer that looked and found nothing, which is the confusion §8.2 exists to prevent. Both branches now go through one shape check, and a payload that fails it is `unverified`. That check asks WHOLE-PAYLOAD questions only — an object carrying a `critical_findings` array — because its answer voids the whole review: one malformed `line` among several, which is what the free-form branch exists to tolerate, discarded a finding that had located the planted defect. Placement is the scorer's question, one finding at a time. A 🔴 it cannot place — no `file` (the schema permits that for a cross-cutting finding) or a `line` that is not a line number — is read for the site instead of discarded: a catch when the finding's own text (`summary`/`reasoning`/`grounding`) names the planted file, a miss when it names nothing anywhere. On a decoy it is the false flag it would be, because an unplaceable 🔴 blocks the merge just as a placed one does.
  - `scripts/render-benchmark.mjs` + `scripts/check-benchmark-claims.mjs` — every benchmark number in `README.md`, `benchmark/README.md`, `benchmark/RESULTS.md` and on the site is rendered from `benchmark/results/latest.json` and byte-compared on every PR (wired into CI's `test` job next to the version guard). The two site passages carry no literal numeral at all, each must name the remainder (`BENCHMARK.unverifiedClause`, empty when there is none), and a passage that describes the committed corpus must read its composition off `BENCHMARK.corpusPlanted`/`corpusDecoys` — the scored counts are the SAMPLE, so a cost-stopped run published the fixed public corpus as holding two planted defects and one decoy; the checker fails on any of the three. `readLatest()` is the single door into all four, and it refuses a dry run, a partial suite, a scenario set that is not the committed corpus on disk — as a multiset, because a `Map` keyed by id dedupes in silence and nine extra copies of an already-caught scenario lifted the headline past the corpus while membership held in both directions (`config.suite: 'all'` is a label the file writes about itself — a result missing the six decoys rendered a 14-scenario headline and passed) — totals the file's own per-defect table does not add up to, and a run that scored no planted defect or no decoy at all (a revoked key published "caught 0/0 (0%)", which reads as a reviewer that failed rather than a run that never happened). A headline nothing can re-derive is a headline nobody can refute.
  - `.github/workflows/benchmark.yml` — weekly plus `workflow_dispatch` (seed, cost cap, suite), one run at a time so two triggers cannot spend two caps against the same key, key-gated to a warning rather than a red cross, fork-gated, opening a PR against `dev` with the re-rendered result. A dispatched subset is scored to scratch and left unpublished. The cap has one owner, the runner's own default; the workflow passes `--cost-cap-usd` only when an operator chose one.
  - **The three workflow templates are held to one invocation contract.** The runner reads the allow-list, the model and the thinking budget out of whichever template `clud-bug init` would render here, and every other assertion passes off whichever file it happens to read because the three ship the same values today — so a `--allowedTools` entry added to `workflow-ts.yml.tmpl` alone passed the whole suite. A test now asserts all three fields identical across `workflow.yml.tmpl` / `-ts` / `-py` and names the diverging file.
  - **`benchmark/README.md`'s launch-gate criteria 1 and 5 are reworded against §4.7, not annotated as stale.** Criterion 1 asked for "a reproduction transcript" and criterion 5 for a green/red probe on every invariant-touching PR — both describe the reviewer-execution capability clud-bug#281 deleted, and criterion 5's mechanism (R6-action's sandboxed probe job) is cancelled outright. Criterion 1 now asks for §4.7 grounding (a quoted diff line, a named CI check that already ran and failed, or the named invariant); criterion 5 is the CI-evidence floor `src/core/ci-checks.ts` implements. The "these two are stale" caveat that stood in for the rewrite is deleted, and so is "This dir starts at 3 scenarios" — the generated block one screen above it owns the corpus size.

  **The published score is the last real measurement, not an automated one yet**: `latest.json` carries the hand-scored 3-reviewer panel of #219 with `invocation: "manual panel (pre-automation)"` and a null recipe hash, and the rendered wording says so in as many words. The first scheduled run replaces it. §8.2 also requires planted defects to be **generated rather than drawn from a fixed set** — this corpus is fixed and public, so it measures regression, not liveness; that half is tracked separately and the wording says that too.
...
- **The pre-push mechanical check now BLOCKS on a missing or dishonest test-suite declaration, and `init` detects + asks up front (#319).** #276 shipped the surface but deliberately left half of SPEC 2.0 §6.7's declaration matrix undone: "Shipping 'block every push' before the setup flow that collects the declaration exists would wedge every repo that upgrades" — so every missing-declaration path *allowed and reported* rather than blocking. That gap is closed:

  - **Suite detection** (`hooks.ts`, read from the same base ref as the declaration — never the working tree): common test-file conventions across languages (`*.test.*`, `*.spec.*`, a `tests/`/`__tests__/`/`spec/` directory, `*_test.go`, `test_*.py`, `*_spec.rb`), plus a real (non-placeholder) `package.json` `scripts.test`. Either signal is enough. No network I/O — the hook's synchronous, must-never-hang property from #276 is unchanged.
  - **The declaration matrix now blocks exactly where §6.7's table says it must**: nothing declared always blocks (whether or not a suite is detected — detection only changes which reason is printed); `"tests": "none"` blocks *only* when a suite is actually detected ("the declaration contradicts the repository"); a declared command still just runs, regardless of detection, exactly as before.
  - **The bootstrap exemption** — "A push whose only change is adding that declaration MUST be allowed, or the config that unblocks pushing can never itself be pushed" — is implemented narrowly: a push whose entire diff touches nothing but `.claude/skills/.clud-bug.json` is allowed even when the *old* base-ref declaration was missing or `"none"`.
  - **`clud-bug init`** now detects a suggested test command (a real `package.json` `scripts.test`) and asks for the declaration before completing setup (§6.7: "Setup MUST ask, and MUST NOT complete without an answer") — `--accept-all` auto-accepts a detected command but never guesses `"none"` with nobody looking; with nothing detected it leaves the declaration unset and warns loudly instead. `clud-bug update` cannot prompt (it runs unattended in the self-update Action) but warns when a pre-push hook is installed with no declaration.
  - The two failure kinds stay apart, per §6.7: a missing/contradicted declaration is a **repo-config state** and blocks; the hook being unable to resolve its own configuration at all (no remote, no default branch, no `node`) is still an **infra failure** and still fails open, unchanged from #276.
...
- **An installed commit-review hook had no way to tell it was running stale gating logic (#312), which is why #303's misfire report was actually a repo running a hook that predated its own fix (#245/#240 — merged three weeks before #303 was filed).** The review *recipe* the hook fetches auto-updates every fire (`npx clud-bug@next` re-resolves fresh each call); the hook's own trigger/gating *script* is a static string baked into `.claude/settings.json` at `init`/`update` time and does not — and the one nudge that already exists for a stale install, `update-notifier.ts`'s interactive check, never runs in the hook's non-interactive, detached (`async: true`) context. `buildCommitReviewCommand` (`hooks.ts`) now appends a one-line freshness note to whatever it surfaces, computed from two local file reads only — the version this repo's own `.claude/skills/.clud-bug.json` was last stamped with, and the newest version update-notifier's existing once-a-day background refresh already cached in `~/.cache/clud-bug/update-check.json` — **never a new network call of its own**. The comparison is directional and prerelease-aware (the same ordering `isNewerVersion` already implements in `update-notifier.ts`, ported into the hook's own `node -e` parser), not plain string inequality — a `!=` check nudges backwards whenever the installed version is already ahead of a cache that hasn't caught up to it yet, a routine race given the manifest is stamped immediately by `update` while the cache refreshes at most once a day. Either file missing (never run, or a repo old enough to predate the field) stays silent, the same fail-open rule every other gate in this hook already follows. Takes effect the next time an already-installed repo runs `init`/`update` and picks up the regenerated script — same bootstrap constraint every hook fix in this file has always had. The pre-existing *interactive* nudge (`update-notifier.ts`, unrelated code path — runs on any `clud-bug` command, not just the hook) carried the same overclaim in its own words — "max-mode hooks already auto-update" — true only of the recipe, never the trigger script; narrowed to say exactly that instead.
- **Regression coverage for #303 ("hook fires without a commit") — the underlying bug was already fixed by #240/#245, this closes a gap in its test coverage.** #240/#245 (`0bfabed`) replaced text-matching on the triggering command with a gate on git's own reflog: fire only when HEAD moved *and* the reflog says the move was a commit-creating action (`commit*|rebase*|pull*|cherry-pick*|revert*|merge*`), which already covers every firing #303 listed (read-only `git log`/`gh` calls, an `Edit` tool call, a worktree commit misattributed to the parent session). What wasn't directly exercised: HEAD moving to a **new, never-before-seen** sha with **no commit involved at all** — `git reset --hard HEAD~1`, and checking out to another branch and back. `test/hooks.test.js` now pins both: the hook stays silent (status 0, empty stdout) in either case, and a mutation on the reflog allowlist (adding `reset*`/`checkout*`) demonstrably flips both tests red.
- **`clud-bug update`/`init` could splice a rendered block into a marker pair quoted inside a FENCED code example — or, given an unrelated fence elsewhere in the file, lose track of the real live block entirely and append a duplicate on every run (#253 residual; #253's original defect, `update` silently deleting or welding hand-written prose next to a marker block, was already fixed by `c90dba7`/#293).** `upsertBlock`/`removeBlock` (`agents-md.ts`) matched the first `<!-- clud-bug-start -->`/`<!-- clud-bug-end -->` pair found anywhere in the file with no awareness of ```` ``` ````/`~~~`-fenced regions, so an AGENTS.md section documenting the marker syntax by *showing* it (a fenced sample containing the literal markers) got overwritten as if it were the live block. Fence detection now follows CommonMark's own pairing rule: a closer must share its opener's character, in a run at least as long, or it's not a closer at all — and an opener that never gets one (an unterminated fence) is never treated as fenced, rather than assumed to extend to end of file. That last part matters beyond the quoted-example case: treating an unterminated fence as open-to-EOF, or accepting any fence-looking line as any other fence's closer regardless of character, could mis-scope a real live block sitting past an unrelated unclosed or mismatched fence as "inside a fence" — so `update` couldn't find it to replace and appended a fresh duplicate each run instead. Pinned on the actual file bytes `update` produces, across repeated runs. Skipping a fenced pair is also not the same as skipping the bytes it matched, which is where the first cut of this fix still lost the live block: the non-greedy `[\s\S]*?` run stops at the first end marker *anywhere* after its start marker, so a fenced example quoting a **lone** `<!-- clud-bug-start -->` — the shape `README.md` itself writes in prose — matched all the way down to the live block's own end marker, and rejecting that match while letting the scan resume where the regex engine had left it put the live pair behind the cursor. `upsertBlock` appended a fresh block on every run (three runs, three start markers, the live one stale under all of them); `removeBlock`, built on `String.replace` — which cannot rewind — removed *nothing*, leaving a per-tool file carrying a full copy of the block body next to its redirect stub, the §1.2 violation the strip exists to prevent. Both now route through one primitive, `liveBlockSpans()`, which resumes from just past the **quoted start marker** rather than past the end marker the rejected match consumed, and returns every live span at once (`upsertBlock` takes the first, `removeBlock` takes all of them) — so the two can no longer disagree about which bytes are clud-bug's.
- **Closes #253: the four residuals the 2026-09-15 status comment left open, plus the follow-up #271 deferred to this lane.** All four reproduced against the actual bytes `update` produces before being fixed:
  1. A marker pair quoted as **inline code** — `` `<!-- clud-bug-start --> ... <!-- clud-bug-end -->` `` in a sentence, not a fenced block, the exact shape `README.md:43` wrote — was still read as live; `agents-md.ts` now excludes a start marker sitting inside an inline code span the same way it already excluded a fenced one, via a new `inlineCodeSpans()` pass. A bare marker with **no** quoting at all (no backticks, no fence) is still read as live on purpose — only the two quoted forms are documentation, and `README.md:43` is fixed to actually quote its mention.
  2. A fence-looking line hidden inside raw HTML — a `` ``` `` starting a line inside a `<!-- ... -->` comment, which CommonMark never parses as markdown — could shift `fenceRanges()`'s idea of where a fence starts or ends, so a fence ends up **straddling** a candidate marker span (starting inside it, ending outside, or the reverse) rather than cleanly containing or missing it. Any span whose fence overlap has that shape is now rejected outright, rather than trusted either way.
  3. `removeBlock` emitted one separator per **span**, not per **gap** — two touching live blocks (only blank lines between them) left the blank lines from both sides stacked instead of collapsed to the one gap they actually left. Span removal extents are now merged before separators are decided.
  4. **Migration ruling:** a file already carrying more than one live block — damage from the shipped duplicate-append bug #253 itself reported — used to have `update` refresh the first and silently leave every later copy stale forever. `update`/`init` now collapse to exactly one (keep the first, already the slot that gets refreshed; remove the rest, closing gaps the same way `removeBlock` does) and print a one-line notice naming the file. The old contract this replaces — "the second block is not ours to rewrite" — was itself the bug; `test/agents-md.test.js` is re-pinned to the collapse instead.

  Separately, three follow-ups #271 (`clud-bug config` — one schema owns every setting, SPEC 2.0 §1.6) deferred to this lane, now landed:
  - **One owner for the §6.7 test declaration.** The pre-push hook's `tests` resolution and suite-detection heuristics were a second, hand-typed copy of `src/core/tests-declaration.ts`'s `readTestsDeclaration` (`.logmind/config.yml` wins where it declares one, else `.clud-bug.json`) and `src/core/detect.ts`'s `TEST_FILE_PATTERN`/`detectTestSuite`. The hook is a *generated* shell script, so it now `import`s the shared pattern strings (`TEST_FILE_PATTERN`, `NPM_INIT_TEST_PLACEHOLDER_PATTERN`, `LOGMIND_TESTS_LINE_PATTERN`) and renders them into the generated `node -e`/`grep` text at generation time instead of hand-copying them — the parity test in `test/config-parity.test.js` now pins the *generated script's own bytes* equal to the export, and `test/pre-push-hook.test.js` proves the logmind-wins precedence end-to-end against real git state. The §6.7 bootstrap exemption (a push that only adds the declaration is always allowed) now also recognizes `.logmind/config.yml` as one of the files the declaration can legitimately live in.
  - **`review.trigger` is real.** `init --hook-trigger <commit|push|both>` now writes `reviewTrigger` through the #271 schema (`CONFIG_KEYS` already declared it), and `clud-bug update` reconciles the *installed hooks* to that value — adding a surface it now names and isn't there yet, removing one it no longer names (restoring a chained foreign pre-push hook if `init` preserved one) — rather than treating "a hook file happens to exist" as a second, driftable record. Reconciliation only fires for a repo that already has at least one local surface installed, so a bare `config set review.trigger` can never become the first install of a surface `--with-hooks` was never asked to scaffold; a manifest that has never recorded a trigger (pre-#253) keeps the original refresh-only contract. `init`'s own no-flag fallback (a bare `clud-bug init --with-hooks` re-run) had the same residual: it resolved the trigger from the hook *files* on disk alone, so a `clud-bug config set review.trigger push` on a commit-only repo was silently reverted back to `commit` on the next bare re-run — the manifest's own value, once set, is now checked first there too (`resolveReviewTrigger`, shared with `update`'s reconciliation above), and only falls back to the file-based guess when the manifest has never recorded one. That fix made the *manifest* correct on a bare re-run but left `init`'s own hook reconciliation add-only: switching surfaces (a `config set review.trigger` since the last `init`, or an explicit `--hook-trigger` naming a different one) could leave the *old* surface's hook installed alongside the new one — a stale commit-review entry in `settings.json`, or a stale blocking `pre-push` file. `init` now reconciles both the same three ways `update` already does — add/refresh what's now named, remove what no longer is — through two primitives (`resolveCommitHookMerge`, `removePrePushHookFile`) extracted so `update`'s own reconciliation shares them too, rather than each caller re-implementing removal on its own.
  - **Every surface pointing at a hand-edit now points at the command instead** (SPEC §1.6:260: "nobody should need to hand-edit the file"). The three pre-push `PUSH BLOCKED` messages, `update`'s missing-`tests` advisory, and the generated `AGENTS.md` block's strict-mode line all now say `clud-bug config set <key> <value>` — except strict mode itself, which SPEC §1.6:262 makes a person's call: the block now says so explicitly and that `config set review.strict_mode` refuses.
